<table id="userlist_tbl" class="">
    ...
    <tr id="userlist_row" class="{cycle values="odd,even"}" onclick="xajax_ownerDetailDisplay({$owners[name].id});">
        <td id="userlist_data1">{$owners[name].login}</td>
        <td id="userlist_data2">{$owners[name].name}</td>
        <td id="userlist_data4">{$owners[name].email}</td>
        <td id="userlist_data5">{$owners[name].status}</td>
    </tr>
	{/section}
